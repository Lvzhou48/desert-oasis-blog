...
公开分支 `public/desert-oasis-blog` 已由已验证的网站快照创建为无父提交的根分支；其作者和提交者身份均为作者提供的 GitHub `noreply` 邮箱。原开发分支 `feature/desert-oasis-blog` 保留为本地备份，**不得推送该分支或任何继承其旧历史的分支**；禁止使用 `--all` 或 `--mirror`。

外部发布前仍必须：

1. 再次核验待推公开分支没有父提交，且全部作者与提交者身份都是该 `noreply` 邮箱。
2. 单独取得作者对创建公开远端仓库和推送公开分支的明确批准。
3. 推送时只推送经过核验的公开分支；不要推送旧开发分支或包含其历史的引用。
